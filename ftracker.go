...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	averagSpeedRunning := meanSpeed(action, duration) * kmhInMsec
	return (runningCaloriesMeanSpeedMultiplier * averagSpeedRunning * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	meanSpeedInKmh := swimmingMeanSpeed(lengthPool, countPool, duration)
	swimCal := (meanSpeedInKmh + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimCal
}
